...
def reversal_analysis(sessions, pre_post_trials=[-15,40], fig_no=1, return_fits=False, clf=True,
                      cols=0, by_type=False, title=None, plot_tau=False):
    ...
    if return_fits:
        ...
        if by_type:
            ...
            _plot_mean_choice_trajectory(choice_trajectories_rr, per_subject_rr, pre_post_trials, colors[0])
            plt.subplot(1,3,2)
            ...
            _plot_mean_choice_trajectory(choice_trajectories_tr, per_subject_tr, pre_post_trials, colors[0])
            plt.subplot(1,3,3)
        ...
        _plot_mean_choice_trajectory(choice_trajectories_br, per_subject_br, pre_post_trials, colors[0])
...
def _plot_mean_choice_trajectory(choice_trajectories, per_subject_ave_traj, pre_post_trials, col='b'):
    x = np.arange(pre_post_trials[0], pre_post_trials[1])
    per_sub_mean = np.mean(per_subject_ave_traj,0)
    per_sub_SD   = np.std(per_subject_ave_traj,0)
