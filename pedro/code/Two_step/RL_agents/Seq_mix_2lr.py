...
class Seq_mix_2lr(RL_agent):
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        for i, (c, s, o) in enumerate(zip(choices[:-1], second_steps, outcomes)): # loop over trials.
            ...
            V[:,:,i+1] = V[:,:,i] * (1.-decV) # Second step forgetting.

            Q[c,i+1] = (1.-alpQ)*Q[c,i] + alpQ*((1.-lbd)*V[c,s,i] + lbd*o) # First step TD update.
            V[c,s,i+1] = (1.-alpV)*V[c,s,i] +  alpV*o # Second step TD update.
