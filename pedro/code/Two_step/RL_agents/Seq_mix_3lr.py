...
class Seq_mix_3lr(RL_agent):
    ...
    def __init__(self, kernels = ['bias', 'ck', 'ssk']):
        self.name = 'Seq_mix_3lr'
        self.param_names  = ['alpQ1', 'decQ1', 'alpQ2', 'decQ2', 'alpV', 'decV', 
                             'lbd', 'alpT', 'decT', 'G_td', 'G_mb']
        self.param_ranges = ['unit']*9 + ['pos']*2
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpQ1, decQ1, alpQ2, decQ2, alpV, decV, lbd, alpT, decT, G_td, G_mb = params_T[:11]
        ...
        for i, (c, s, o) in enumerate(zip(choices[:-1], second_steps, outcomes)): # loop over trials.
            ...
            Q1[n,i+1]   = Q1[n,i]   * (1.-decQ1) # First step forgetting.
            Q2[:,:,i+1] = Q2[:,:,i] * (1.-decQ2) # Second step forgetting.
            V[r,i+1]    = V[r,i]    * (1.-decV)  # Second step forgetting.

            Q1[c,i+1]   = (1.-alpQ1)*Q1[c,i]    + alpQ1*(Q2[c,s,i]*(1.-lbd)+lbd*o) # First step TD update.
            Q2[c,s,i+1] = (1.-alpQ2)* Q2[c,s,i] + alpQ2*o # Second step TD update.
