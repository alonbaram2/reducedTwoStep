...
class MBi_dec(RL_agent):
    'Model based agent with forgetting and initial transition probabilities as model-parameter.'

    def __init__(self, kernels = ['bs', 'ck', 'rb']):
        self.name = 'MBi_dec'
        self.param_names  = ['alpV', 'decV', 'alpT' , 'decT', 'initT', 'iTemp']
        self.param_ranges = ['unit', 'unit', 'unit' , 'unit', 'unit' ,'pos'   ]
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpV, decV, alpT, decT, initT, iTemp = params_T[:6]   
