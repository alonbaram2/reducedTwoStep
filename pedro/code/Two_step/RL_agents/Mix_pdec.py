...
class Mix_pdec(RL_agent):
    '''Mixture agent with decays.  Target for value decay is paramiterised.'''

    def __init__(self, kernels = ['bias', 'ck', 'ssk']):
        self.name = 'Mix_pdec'
        self.param_names  = ['alpQ', 'decQ', 'tarQ', 'lbd' , 'alpT', 'decT', 'G_td', 'G_mb']
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpQ, decQ, tarQ, lbd, alpT, decT, G_td, G_mb = params_T[:8]
        ...
        for i, (c, s, o) in enumerate(zip(choices[:-1], second_steps, outcomes)): # loop over trials.
            ...
            Q[n,i+1] = Q[n,i] - decQ*(Q[n,i]-tarQ) # First step forgetting.
            V[r,i+1] = V[r,i] - decQ*(V[r,i]-tarQ) # Second step forgetting.
