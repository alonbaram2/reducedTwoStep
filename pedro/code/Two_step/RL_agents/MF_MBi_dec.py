...
class MF_MBi_dec(RL_agent):
    'Mixture agent with forgetting and initial transition probabilities as model-parameter'

    def __init__(self, kernels = ['bs', 'ck', 'rb']):
        self.name = 'MF_MBi_dec'
        self.param_names  = ['alpQ', 'decQ', 'lbd' , 'alpT', 'decT', 'initT', 'G_td', 'G_mb']
        self.param_ranges = ['unit', 'unit', 'unit', 'unit', 'unit', 'unit' , 'pos' , 'pos' ]
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpQ, decQ, lbd, alpT, decT, initT, G_td, G_mb = params_T[:8]
        ...
        T[:,0] = [initT,1.-initT] # Initialize first trial transition probabilities.
