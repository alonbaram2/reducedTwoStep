...
def per_subject_fit(sessions, agent):
    ...
    for sID in sorted(set([s.subject_ID for s in sessions])):
        ...
        subject_fit = fit_population(subject_sessions, agent)
        subject_fit['sID'] = sID
        subject_fits.append(subject_fit)
    return subject_fits


def longditudinal_fit(experiment, agent, epoch_len=1):
    ...
    epoch_start_days = list(range(1,experiment.n_days + 1, epoch_len))
    longdit_fit = []
    for start_day in epoch_start_days:
        epoch_sessions = experiment.get_sessions('all',list(range(start_day,start_day + epoch_len)))
        epoch_fit = fit_population(epoch_sessions, agent)
        epoch_fit['start_day'] = start_day
        longdit_fit.append(epoch_fit)
    return longdit_fit
...
def fit_session_con(session, agent, repeats=10, verbose=False):
    '''ML fit of session using constrained optimisation. '''
    fit_func =  partial(_neg_likelihood, session=session, agent=agent)
    bounds = [{'unc':(None, None), 'unit':(0.,1.), 'pos':(0.,1000.)}[param_range]
    ...
    for r in range(repeats): # Number of fits to perform with different starting conditions.
        ...
        fits.append(op.minimize(fit_func, init_params_T, bounds=bounds, options={'disp': verbose}))

    fit = fits[np.argmin([f['fun'] for f in fits])]  # Select best fit out of repeats.

    return {'params_T'   : fit['x'],
            'prob'    : - fit['fun'], 
